...
**2026-09-21:** regla 11 del validador (`CollectionUsageChecker`): toda colección usada con `Dashboard.data` debe estar declarada en el manifiesto; detecta literales y constantes simples e indica la línea. Motivo: el dashboard «Plan MKT Vence Tudo 2027» usaba `actividades` sin declararla y no podía crear actividades. El contenedor muestra ese error arriba del dashboard. Prompt de generación ampliado con los errores que rompen el guardado y una verificación de entrega.

**2026-09-18:** nombre y descripción de cada dashboard editables por el admin (el nombre sobrevive a una nueva versión del archivo; la descripción se ve en el inicio y en la cabecera). Captura compatible con el sandbox: html2canvas falla en el iframe sin allow-same-origin (SecurityError al leer el iframe hijo), así que el preámbulo define `window.html2canvas` y `Dashboard.capture` con una implementación por SVG foreignObject (`resources/js/dashboard/captureShim.ts`, inyectada con `toString()`); arregla exportar PDF, copiar imagen y las fotos de traslados sin tocar los dashboards.

**Divisiones y grupos (2026-09-16):** modelo organizativo para usuarios y dashboards. Tablas `divisions`, `groups` y cuatro pivotes; `dashboards.icon` y `dashboards.visible_to_all` (backfill a `true` para los dashboards existentes). Un usuario pertenece a varias divisiones y a grupos de ellas; un dashboard se asigna a divisiones enteras y/o grupos, o a "toda la empresa"; sin asignación sólo lo ven los super administradores. Regla única en `Dashboard::scopeVisibleTo` (usada por listado, detalle, parámetros, datos e historial). Endpoint `GET /api/menu` arma el menú por división y grupo. Menú lateral colapsable a rail de íconos (set fijo de 24 SVG inline, iniciales como respaldo), recordado por navegador. Administración: Divisiones, Visibilidad por dashboard, membresías en el formulario de usuario. Corregido el bug de los botones de la tabla de dashboards que perdían el id en la URL. 23 tests nuevos (182 en total).

**Cambio de alcance (2026-09-09):** los dashboards son aplicaciones con interfaz propia que cargan datos compartidos por todos los usuarios (ejemplo real: `kit/ejemplos/traslado-maquinas-eventos.html`). Se agregó un segundo mecanismo de persistencia, **colecciones de registros** (`Dashboard.data`, tablas `dashboard_records` y `dashboard_record_history` con triggers), con concurrencia por registro y versión, sincronización por sondeo cada 10 s, carga inicial desde el archivo sólo con la colección vacía, borrado por cualquier usuario y restauración sólo para super administradores. La vista del usuario ya no muestra panel de parámetros: se usa la interfaz del dashboard. El sandbox permite diálogos, descargas y ventanas, y delega el portapapeles (con puente `Dashboard.clipboard` como respaldo). Los parámetros escalares siguen disponibles como mecanismo opcional. Decisiones tomadas con Boris: borrado libre, datos iniciales desde el archivo, restaurar sólo admin, 5000 registros por colección y 256 KB por registro, sondeo de 10 s.

**Estado (Semana 4):** administración completa y verificada en navegador: usuarios (alta, edición, desactivación; sin borrado), escenarios por usuario (matriz usuarios × parámetros con origen del valor), historial completo filtrable, edición de valores base con el mismo panel, historial propio del usuario. Kit entregado en `kit/` (ESPECIFICACION, PLANTILLA-PROMPT, GUIA-OPERATIVA, dashboard-referencia). CI en GitHub Actions contra MySQL 8.4. Guía y ejemplos de despliegue en `deploy/`. Correo de restablecimiento en español. 144 tests. Pendiente del cliente: datos del VPS (SO, PHP/MySQL instalados, dominio, SMTP) para ejecutar el despliegue.

**Estado (Semana 3):** motor de parámetros completo y verificado: 131 tests (incluidos los cinco obligatorios del punto 12), y en navegador el panel guarda con debounce, el iframe se redibuja, los resets borran la fila y el historial se llena por trigger. Detalles: `DELETE /api/dashboards/{id}/params` (restablecer todo), `?scope=base` en los DELETE para el super administrador, ruta `/admin/dashboards/{id}/base` que reutiliza la vista con scope base, y `dashboards:prune-orphans {--dry-run} {--dashboard=}`.

**Estado (Semanas 1 y 2):** completadas y verificadas (104 tests; el dashboard de referencia se publica por la API y se renderiza en el iframe sandbox con el puente postMessage funcionando en navegador). Ajustes de la Semana 2: el extractor ignora comentarios HTML; la medición de altura del preámbulo usa el body (documentElement.scrollHeight genera un bucle con el contenedor); `Dashboard.setHeight()` sin argumento mide solo; se agregaron `Dashboard.setParam` y `Dashboard.reportError` para emitir `param:change` y `dashboard:error`; endpoint extra `POST /api/admin/dashboards/preview` para validar sin guardar y mostrar el diff antes de confirmar.
