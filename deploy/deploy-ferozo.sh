...
rsync -rlptDz --delete -e "$RSYNC_RSH" \
  ...
  ./ "$REMOTE:$APP_DIR/"
# La propiedad se corrige acá mismo, antes de composer y de las cachés: si un paso posterior falla,
# los archivos ya son de $OWNER y Apache sigue sirviendo build/ a través del enlace.
"${SSH[@]}" "$REMOTE" "chown -R $OWNER '$APP_DIR'"
