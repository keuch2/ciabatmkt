    ...
    public function archive(Dashboard $dashboard, ?User $actor): string
    {
        $payload = [
            ...
            'records' => DB::table('dashboard_records')->where('dashboard_id', $dashboard->id)->orderBy('collection')->orderBy('created_at')->get()
                ->map(fn ($r) => ['collection' => $r->collection, 'id' => $r->record_id, 'data' => json_decode($r->data, true), 'version' => (int) $r->version, 'updated_at' => (string) $r->updated_at, 'updated_by' => $r->updated_by])->all(),
            'record_history' => DB::table('dashboard_record_history')->where('dashboard_id', $dashboard->id)->orderBy('changed_at')->get()
                ->map(fn ($h) => ['collection' => $h->collection, 'id' => $h->record_id, 'action' => $h->action, 'version' => (int) $h->version, 'old_data' => json_decode($h->old_data ?? 'null', true), 'new_data' => json_decode($h->new_data ?? 'null', true), 'changed_by' => $h->changed_by, 'changed_at' => (string) $h->changed_at])->all(),
            'param_values' => DB::table('param_values')->where('dashboard_id', $dashboard->id)->get()
                ->map(fn ($p) => ['param_id' => $p->param_id, 'user_id' => $p->user_id, 'value' => json_decode($p->value, true), 'updated_by' => $p->updated_by, 'updated_at' => (string) $p->updated_at])->all(),
        ];
